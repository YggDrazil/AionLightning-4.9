...
import com.aionemu.packetsamurai.utils.NpcRiftExporter;
...
public class Main implements IUserInterface {
	...
	public void init() {
		...
		_frame = new JFrame("Packet Samurai [Aion-Lightning - 4.9.x EU Edition] reworked for AionGer");
		...
		//NpcRiftExporter
		JMenuItem exportRifts = new JMenuItem("Export Rift Spawns");
		exportRifts.setActionCommand("ExportRiftSpawns");
		exportRifts.addActionListener(_menuListener);
		_utilsMenu.add(exportRifts);				
		//NpcTestExporter
	...
	private void exportRiftSpawns() {
		if (getViewerTab().getComponentCount() > 0) {
			...
			if (pane != null) {
				String sessionName = pane.getGameSessionViewer().getSession().getSessionName();
				FastList<DataPacket> packets = pane.getGameSessionViewer().getSession().getPackets();
				new NpcRiftExporter(packets, sessionName).parse();
			}
		}
	}		
	//NpcTestExporter
	...
	public class MenuActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent ev) {
			...
			}else if (actionCmd.equals("Flush"))
			{
				...
				exportNpcSpawns();
			} else if (actionCmd.equals("ExportRiftSpawns")){
				exportRiftSpawns();				
