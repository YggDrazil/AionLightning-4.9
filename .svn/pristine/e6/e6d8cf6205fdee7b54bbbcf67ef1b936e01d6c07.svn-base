...
public class Equip extends AdminCommand {
    ...
    private void plume(Player admin, Player player, int plume) {
        ...
        if (player == admin) {
            ...
            PacketSendUtility.sendMessage(admin, "equipped plume by the Player " + player.getName()
                    + " were enchanted to " + plume);
            PacketSendUtility.sendMessage(player, "Admin " + admin.getName() + " enchanted your equipped plume to level "
                    + plume);
        }
	}
