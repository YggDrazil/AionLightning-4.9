...
public class TeleportService2 {
    ...
    private static final int HOTSPOT_DEFAULT_DELAY = 10000;
     ...
     * @param player
     */
    public static void useHotspotTeleport(HotspotTeleportTemplate template, int teleportGoal, Player player, int kinah, int level) {
    ...
    public static void cancelHotspotTeleport(Player player)
    {
        if(player.isHotspotCanceled())
            return;

        player.setHotspotTeleportCanceled(true);
    ...
    private static void sendLoc(final Player player, final int mapId, final int instanceId, final float x, final float y, final float z, final byte h, final int teleGoal, int id) {
        ...
        int delay = HOTSPOT_DEFAULT_DELAY;

        PacketSendUtility.sendPacket(player, new SM_HOTSPOT_TELEPORT(player, teleGoal, id));//id = 1
        player.unsetPlayerMode(PlayerMode.RIDE);
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                if(player.isHotspotCanceled()) {
                    player.setHotspotTeleportCanceled(false);
                    return;
                }
                ...
                PacketSendUtility.sendPacket(player, new SM_HOTSPOT_TELEPORT(player, teleGoal, 3, 600));
