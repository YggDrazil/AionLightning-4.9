    ...
    public static void onDialogSelect(int dialogId, final Player player, Npc npc, int questId, int extendedRewardIndex) {
        ...
        if (questId == 0) {
            switch (DialogAction.getActionByDialogId(dialogId)) {
                ...
                case DEPOSIT_CHAR_WAREHOUSE: { // warehouse 
                    switch (titleId) {
                        case 315008:
                        case 350417:
						case 358039:
                ...
                case EXTEND_INVENTORY: { // expand cube (2.5)
                    //CubeExpandService.expand(player, npc); //TODO
                    break;
                }
                ...
                case EDIT_GENDER: { // (2.5)
					...
					if (player.getInventory().getItemCountByItemId(169650000) > 0 || //Plastic Surgery Ticket
						...
						player.getInventory().getItemCountByItemId(169690001) > 0 || //Plastic Surgery Ticket
						player.getInventory().getItemCountByItemId(169691000) > 0) { //Partial Plastic Surgery Item Exchange Coupon
                            check_ticket = 1;
                        }
                    }
                ...
                case ITEM_UPGRADE: // item purification
                    ...
                    break;
                case STIGMA_ENCHANT_WINDOWS: //Stigma enchant 4.8 Dialog 
                ...
                case SETPRO3:
					switch (npc.getNpcId()) {
						case 804576:
							if (player.getInventory().getItemCountByItemId(185000215) > 0) {
								QuestService.addNewSpawn(600090000, 1, 855263, (float) 791.34467, (float) 489.277, (float) 143.47517, (byte) 27);
								return;
							}
					} //ONLY A EXTREM TEMP FIX !!!!!!!!!!!
                    ...
                    if (portalPath != null) {
                        ...
                        if (loc != null) {
                            TeleportService2.teleport(template, loc.getLocId(), player, npc,
                                    npc.getAi2().getName().equals("general") ? TeleportAnimation.JUMP_ANIMATION : TeleportAnimation.BEAM_ANIMATION);
                        }
                    }
                    break;
                default:
