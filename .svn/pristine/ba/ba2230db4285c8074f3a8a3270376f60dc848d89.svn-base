...
public class ArgentManorInstance extends GeneralInstanceHandler {
	 ...
	 */
	private List<Integer> npcCheckList = new ArrayList<Integer>(Arrays.asList(
			237180, 237181, 237182, 237186, 237188, 237190, 237191, 237192, 237195, 237196,
			237197, 237198, 237199, 237200, 237202, 237193, 237194));
	...
	private void startPrepareTimer() {
		if (timerPrepare == null) {
			timerPrepare = ThreadPoolManager.getInstance().schedule(
					new Runnable() {
						@Override
						public void run() {
	...
	private void startMainInstanceTimer() {
		...
		timerInstance = ThreadPoolManager.getInstance().schedule(
				new Runnable() {
					@Override
					public void run() {
						onTimerEnd();
