...
public class MySQL5BrokerDAO extends BrokerDAO {
            ...
            public void handleRead(ResultSet rset) throws SQLException {
                while (rset.next()) {
                    ...
                    brokerItems.add(new BrokerItem(item, itemId, itemPointer, itemCount, itemCreator, price, seller, sellerId,
                            itemBrokerRace, isSold, isSettled, expireTime, settleTime, isPartSale));
                }
            }
        });
    ...
    private boolean insertBrokerItem(final BrokerItem item) {
        boolean result = DB
                .insertUpdate(
                        "INSERT INTO `broker` (`item_pointer`, `item_id`, `item_count`, `item_creator`, `seller`, `price`, `broker_race`, `expire_time`, `settle_time`, `seller_id`, `is_sold`, `is_settled`, `is_partsale`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        new IUStH() {
                            @Override
    ...
    private boolean updateItem(final BrokerItem item)
    {
        boolean result = DB
                .insertUpdate(
                        "UPDATE broker SET `item_count` = ?, `price` = ?, `is_sold` = ?, `is_settled` = ?, `settle_time` = ?, `is_partsale` = ? WHERE `item_pointer` = ? AND `expire_time` = ? AND `seller_id` = ? AND `is_settled` = 0",
                        new IUStH() {
                            @Override
                            public void handleInsertUpdate(PreparedStatement stmt) throws SQLException {
